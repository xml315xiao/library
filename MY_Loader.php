...
class MY_Loader extends MX_Loader
{
    ...
    public function service($service = '', $params = NULL, $object_name = NULL)
    {
        ...
        {
            //Is the services is an array?If so,load every key
        ...
        // Is the services in a sub-folder? If so, parse out the filename and path.
        if (($last_slash = strrpos($service, '/')) !== FALSE)
        {
            // The path is in front of the last slash
            $path = substr($service, 0, ++$last_slash);

            // And the services name behind it
        ...
        {
            throw new RuntimeException('The services name you are loading is the name of a resource that is already being used: '.$object_name);
        }
